...
inline uint32_t generateMask(const uint32_t length) {
...
inline void pulse_delay_loop() {
    ...
    constexpr const uint32_t clock_loop_max = 0x02;
...
inline RegBits generate_reg_bits(const uint32_t value, const uint32_t offset, const uint32_t length) {

    uint32_t generated_msk = util::generateMask(length);
